...
I0 = zeros(p.N, 1);      % 初始感染细胞 (各龄均为0)
V0 = 10;                % 初始胞外病毒
Tc0 =  1e5;               % 初始 Tc 细胞 CD8+T cell memory
...
for i = 1:10
    for j = 1:10
        x0(10*(i-1)+j+200) = (1-10^(-ii))*G_1*prob_A(i)*prob_A(j)+10^(-ii)*data_new_BCR_IgG(10*(i-1)+j);% IgG-BCR
    end
end
for i = 1:10
    for j = 1:10
        x0(10*(i-1)+j+300) = (1-10^(-ii))*G_2*prob_A(i)*prob_A(j)+10^(-ii)*data_new_IgG(10*(i-1)+j);% IgG
    end
end
...
for i = 1:10
    for j = 1:10
        x0(10*(i-1)+j+1300) = (1-10^(-ii))*P_G*prob_A(i)*prob_A(j)+10^(-ii)*data_new_sec_IgG(10*(i-1)+j); % plasma IgG cell
    end
end
...
plot(t_new+1000, Total_I, 'r'); 
hold on
end
saveas(gcf, 'different_a_value_new.fig');