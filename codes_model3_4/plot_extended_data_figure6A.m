...
p.para(15) = 2;% amplification constant of virus antigen
...
p.para(20) = 1e5;
p.para(21) = -0.05;%% decay constant of vaccine
...
[t, y] = ode15s(@(t,y) sys_ode_new_1(t, y, p), t_span, y0, options);
...
T0 = interp1(t,y(:,1),2000);                % 初始易感细胞
for i = 1:400
    I0(i) = interp1(t,y(:,i+1),2000);  
end

V0 = interp1(t,y(:,402),2000);                % 初始胞外病毒
Tc0 = interp1(t,y(:,403),2000);                 % 初始 Tc 细胞 1e5

xx0 = zeros(1803,1);
for i = 1:1803
xx0(i) = interp1(t,y(:,i+403),2000);
end
xx0(1401)= xx0(1401)+1e20;
% p.para(3) = 2.2e17+1e13+5e17; % replenish constant pi 1 
%xx0(1402) = 10;%% vaccine first and then infection

% xx0(1403) = 1e13;

y0 = [T0; I0; V0; Tc0; xx0; cum_death_0]; 
...
[t_new, y_new] = ode15s(@(t,y) sys_ode_new_1(t, y, p), t_span, y0, options);
...
% --------------------------
nexttile; % 替代 subplot
Total_I = sum(y_new(:, 2:p.N+1), 2);
plot(t_new, y_new(:,1), 'b'); hold on;
plot(t_new, Total_I, 'r'); hold off;
title('Cell Populations', 'FontWeight', 'bold'); % 加a) b)序号符合论文习惯
ylabel('Cell Count', 'FontWeight', 'bold');
legend({'Susceptible (T)', 'Infected (I)'}, 'Location', 'best', 'Box', 'off');
grid off; box on; xlim([0 max(t)]);
...
% --------------------------
nexttile;
plot(t_new, y_new(:, p.N+2), 'm');
title('Extracellular Virus (V)', 'FontWeight', 'bold');
ylabel('Virus Titer', 'FontWeight', 'bold');
set(gca, 'YScale', 'log'); % 对数坐标
grid off; box on; xlim([0 max(t)]);
...
% --------------------------
nexttile;
plot(t_new, y_new(:, p.N+3), 'k');
title('Cytotoxic T Cells (Tc)', 'FontWeight', 'bold');
xlabel('Time (h)', 'FontWeight', 'bold');
ylabel('Tc Count', 'FontWeight', 'bold');
grid off; box on; xlim([0 max(t)]);
...
% --------------------------
nexttile;
Cum_Natural = y_new(:, end-2);
Cum_ADCC    = y_new(:, end-1);
Cum_Tc      = y_new(:, end);
% 计算速率
Rate_Natural = diff(Cum_Natural) ./ diff(t_new);
Rate_ADCC    = diff(Cum_ADCC)    ./ diff(t_new);
Rate_Tc      = diff(Cum_Tc)      ./ diff(t_new);
t_mid        = (t_new(1:end-1) + t_new(2:end)) / 2;

plot(t_mid, Rate_Natural, 'k'); hold on;
plot(t_mid, Rate_ADCC,    'b');
plot(t_mid, Rate_Tc,      'r'); hold off;
title('Dynamics of Cell Death', 'FontWeight', 'bold');
xlabel('Time (h)', 'FontWeight', 'bold');
ylabel('Rate (cells/h)', 'FontWeight', 'bold');
legend({'Natural', 'ADCC', 'Tc Killing'}, 'Location', 'northeast', 'Box', 'off');
grid off; box on; xlim([0 max(t)]);
