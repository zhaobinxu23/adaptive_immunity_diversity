...
[t, y] = ode15s(@(t,y) sys_ode(t, y, p), t_span, y0, options);

for i = 1:100

data_new_BCR_IgG(i) = interp1(t,y(:,603+i),1000);  
 
data_new_IgG(i) = interp1(t,y(:,703+i),1000);  
 
data_new_sec_IgG(i) = interp1(t,y(:,1303+i),1000);  
end
...
I0 = zeros(p.N, 1);      % 初始感染细胞 (各龄均为0)
V0 = 10;                % 初始胞外病毒
Tc0 =  interp1(t,y(:,403),1000);               % 初始 Tc 细胞 CD8+T cell memory
...
for i = 1:10
    for j = 1:10
        x0(10*(i-1)+j+200) = G_1*prob_A(i)*prob_A(j)+1e-4*data_new_BCR_IgG(10*(i-1)+j);% IgG-BCR
    end
end
for i = 1:10
    for j = 1:10
        x0(10*(i-1)+j+300) = G_2*prob_A(i)*prob_A(j)+1e-4*data_new_IgG(10*(i-1)+j);% IgG
    end
end
...
for i = 1:10
    for j = 1:10
        x0(10*(i-1)+j+1300) = P_G*prob_A(i)*prob_A(j)+1e-4*data_new_sec_IgG(10*(i-1)+j); % plasma IgG cell
    end
end
...
[t_new, y_new] = ode15s(@(t,y) sys_ode(t, y, p), t_span, y0, options);
